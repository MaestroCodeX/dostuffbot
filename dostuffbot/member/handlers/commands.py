...
command_delete_conv = ConversationHandler(
    entry_points=[MessageHandler(to_filter_regex(texts.DELETE_COMMAND), command_delete)],
    states={
        states.DELETE_CONFIRM: [
            MessageHandler(to_filter_regex(texts.DELETE_COMMAND), command_delete),
...
command_menu_conv = ConversationHandler(
    ...
    states={
        states.CHOOSE_COMMAND_OPTION: [
            MessageHandler(to_filter_regex(texts.DELETE_COMMAND_CONFIRM), command_delete_confirm),
            MessageHandler(to_filter_regex(texts.SHOW_ANSWER), command_show_answer),
        ],
    },
    fallbacks=[MessageHandler(Filters.all, ignore)],
...
command_conversation = ConversationHandler(
    entry_points=[MessageHandler(to_filter_regex(texts.COMMANDS), commands_list)],
    states={
        states.COMMAND_MENU: [command_menu_conv],
    },
    fallbacks=[MessageHandler(Filters.all, ignore)],
)
